...
const char * _spp_server_name = "YardsTechSPP";
...
#define INQ_LEN 0x08 //0x10 //0x03 //needs to be min of 0x03 docs say 0x01 to 0x30. Multiply by 1.28 seconds. So 3 x 1.28 = 3.84 seconds
#define INQ_NUM_RSPS 15 //20 //4 //20
...
String Discovered;
String LastClientAddr;
bool DoingDiscovery = false;
#define INQ_LEN_DISCOVER INQ_LEN
#define INQ_NUM_RSPS_DISCOVER 4 //20

String LogMsg;
bool MsgChanged;

//2.0.0static BTScanResultsSet scanResults;
//2.0.0static BTAdvertisedDeviceCb advertisedDeviceCb = nullptr;
...
BtConnectStatus BtStatus;
bool BtStatusChanged = false;
...
#define MAX_DISCO_ARR_SIZE 30
btDisco DiscoArr[MAX_DISCO_ARR_SIZE];
int DiscoArrLen;
...
//static void AddNameAddress (const char *Name, char *Address)
//{
//  log_i("Looking to add name/address : %s : %s", Name, Address);
//  if (Discovered.indexOf(Address) < 0){
//    Discovered.concat ("\"");
//    Discovered.concat (Name);
//    Discovered.concat ("\";\"");
//    Discovered.concat (Address);
//    Discovered.concat ("\";");
//    log_i("Name/address added : %s", Discovered.c_str());
//  }
//  else {
//    if (Name != NO_NAME) {
//      if 
...
static void AddNameAddress (const char *Name, char *Address)
{
  ...
  for (int i=0; i < DiscoArrLen; i++) {
    if (DiscoArr[i].Address == Addr) {
      Idx = i;
      break;
    }
  }

  if ((Idx < 0) && (DiscoArrLen < MAX_DISCO_ARR_SIZE)) {
    DiscoArrLen++;
    DiscoArr[DiscoArrLen - 1].Name = String (Name);
    DiscoArr[DiscoArrLen - 1].Address = String (Address);
    log_i("Name and address added. Idx : %d", DiscoArrLen);
  }
  else if (Idx >= 0) {
    if ((DiscoArr[Idx].Name == String (NO_NAME)) && (String (Name) != String (NO_NAME))) {
      DiscoArr[Idx].Name = String (Name);
      log_i("Name updated. Idx : %d", Idx + 1);
    }
  }
}
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_SRV_OPEN_EVT://Server connection open
        log_i("ESP_SPP_SRV_OPEN_EVT. Server connected.");
        _spp_server = 1;
    ...
    case ESP_SPP_OPEN_EVT://Client connection open
        ...
        bda2str(param->open.rem_bda, bda_str, 18);
        log_i("Connected device client: %s", bda_str);
        LastClientAddr = bda_str;
  ...
  bool NameAdded = false; 
    ...
    switch(event){
        case ESP_BT_GAP_DISC_RES_EVT: {
            ...
            log_i("Scanned device: %s", bda_str);
            log_i("Number of props: %d", param->disc_res.num_prop);
                ...
                switch(param->disc_res.prop[i].type) {
                    case ESP_BT_GAP_DEV_PROP_EIR:  
                        if (get_name_from_eir((uint8_t*)param->disc_res.prop[i].val, peer_bdname, &peer_bdname_len)) {
                            ...
                            if (DoingDiscovery) {
                              NameAdded = true;
                              AddNameAddress (peer_bdname, bda_str);
                            }
            ...
            if (DoingDiscovery && !NameAdded)
            {
              AddNameAddress (NO_NAME, bda_str);  
            }
                
//2.0.0            if (peer_bdname_len)
//2.0.0                advertisedDevice.setName(peer_bdname);
            esp_bd_addr_t addr;
            memcpy(addr, param->disc_res.bda, ESP_BD_ADDR_LEN);
//2.0.0            advertisedDevice.setAddress(BTAddress(addr));
//2.0.0            if (scanResults.add(advertisedDevice) && advertisedDeviceCb)
//2.0.0                advertisedDeviceCb(&advertisedDevice);
        }
        break;
        ...
        case ESP_BT_GAP_DISC_STATE_CHANGED_EVT:
            ...
            if (param->disc_st_chg.state == ESP_BT_GAP_DISCOVERY_STOPPED) {
              ...
              if (DoingDiscovery) {
                ...
                log_i("DoingDiscovery = false");
                Discovered = "";
                for (int i=0; i < DiscoArrLen; i++) {
                  Discovered.concat ("\"");
                  Discovered.concat (DiscoArr[i].Name);
                  Discovered.concat ("\";\"");
                  Discovered.concat (DiscoArr[i].Address);
                  Discovered.concat ("\";");
                }

                log_i("Discovery done : %s", Discovered.c_str());
              }
...
//    if (_isMaster && esp_bt_gap_register_callback(esp_bt_gap_cb) != ESP_OK) {
    ...
    esp_power_level_t PwrMin;
    esp_power_level_t PwrMax;
    esp_bredr_tx_power_get (&PwrMin, &PwrMax); 
    log_i("Bt power min %d, max %d", PwrMin, PwrMax);
    esp_err_t res = esp_bredr_tx_power_set (ESP_PWR_LVL_P9, ESP_PWR_LVL_P9);
    log_i("Changing Bt power - %d", res); 
...
static bool waitForConnect(int timeout) {
    //VV added this line below - this removes the wait function. If not disabled then it will block wifi whenever it's trying to discover/connect to devices
    return true;
...
bool BluetoothSerial::hasServer(void)
{
    return _spp_server > 0;
}
...
BtConnectStatus BluetoothSerial::GetBtStatus()
{
...
bool BluetoothSerial::HasBtStatusChanged()
{
  return BtStatusChanged;  
}


bool BluetoothSerial::ConnectDone(void)
{
    return BtStatus != btConnecting;
}
...
String BluetoothSerial::ClientAddr(void)
{
  if (hasClient()) {
...
////2.0.0void BluetoothSerial::discoverAsyncStop() {
////2.0.0    esp_bt_gap_cancel_discovery();
////2.0.0    advertisedDeviceCb = nullptr;
////2.0.0}
//
///** @brief      Clears scanresult entries */
////2.0.0void BluetoothSerial::discoverClear() {
////2.0.0    scanResults.clear();
////2.0.0}
//
...
// */
////2.0.0BTScanResults* BluetoothSerial::getScanResults() {
////2.0.0    return &scanResults;
////2.0.0}
...
bool BluetoothSerial::Discover(void)
{
    if (!isReady(true, READY_TIMEOUT)) return false;
    DiscoArrLen = 0;
    Discovered = "";
    DoingDiscovery = true;
    log_i("DoingDiscovery = true");

    disconnect();
    _isRemoteAddressSet = false;
    strncpy(_remote_name, "DISCOVER", ESP_BT_GAP_MAX_BDNAME_LEN);
    _remote_name[ESP_BT_GAP_MAX_BDNAME_LEN] = 0;
    log_i("master : Discover");
    // will first resolve name to address
    #if ESP_IDF_VERSION_MAJOR >= 5
    ...
    if (esp_bt_gap_start_discovery(ESP_BT_INQ_MODE_GENERAL_INQUIRY, INQ_LEN_DISCOVER, INQ_NUM_RSPS_DISCOVER) == ESP_OK) {
        log_i("Starting discovery now ...");
        return waitForConnect(SCAN_TIMEOUT);
    }
    return false;
}
...
bool BluetoothSerial::DiscoverDone(void)
{
    return !DoingDiscovery;
}


String BluetoothSerial::DiscoverResult(void)
{
    return Discovered;
}
